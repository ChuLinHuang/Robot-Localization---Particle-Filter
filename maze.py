...
class Maze(object):
    ...
    def sensor_model(self, coordinates, sensor_limit, orientation = 0):
        ...
        pos_y = y
        d5 = 0
        dx = np.cos(orientation+np.pi/4) * 1 - np.sin(orientation-np.pi/2) * 0
        dy = np.sin(orientation+np.pi/4) * 1 + np.cos(orientation-np.pi/2) * 0
        ...
        pos_y = y
        d6 = 0
        dx = np.cos(orientation-np.pi/4) * 1 - np.sin(orientation) * 0
        dy = np.sin(orientation-np.pi/4) * 1 + np.cos(orientation) * 0
        ...
        pos_y = y
        d7 = 0
        dx = np.cos(orientation+3*np.pi/4) * 1 - np.sin(orientation+np.pi/2) * 0
        dy = np.sin(orientation+3*np.pi/4) * 1 + np.cos(orientation+np.pi/2) * 0
        ...
        pos_y = y
        d8 = 0
        dx = np.cos(orientation-3*np.pi/4) * 1 - np.sin(orientation-np.pi) * 0
        dy = np.sin(orientation-3*np.pi/4) * 1 + np.cos(orientation-np.pi) * 0
        while not self.colide_wall(int(round(pos_y)),int(round(pos_x))) and d8 < sensor_limit:
            pos_x = pos_x + dx
            pos_y = pos_y + dy
            d8 += 1
        #return 8 for addition model.
        ###############
...
class Robot(Particle):
    def __init__(self, x, y, maze, heading = None, sensor_limit = None, noisy = True, measurement_noise=False):
